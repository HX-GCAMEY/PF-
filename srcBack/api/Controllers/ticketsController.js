...
export default class TicketsController{
  ...
  static async purchase(req, res){
    try{
      ...
      const flightData = await FlightsDAO.getFlightByID(ticketsFromBody.flightId);
      ...
      let insertResult;
      if(flight){
        insertResult = await TicketsDAO.addNewFlightTicket(flightData.number, flightData.totalSeats);
      }

      availableTickets = await TicketsDAO.getAvailableTicket(flightData.number);
      if(availableTickets > 0){
        insertResult = await TicketsDAO.addFlightTicket(newTicket, availableTickets);
  ...
  static async cancel(req, res){
    try {
      ...
      const deleteResult = await TicketsDAO.deleteTicket(ticketId, availableTickets);
