...
const Profile = ({navigation}) => {
          ...
          <TouchableOpacity key={index} onPress={() => setCategoryIndex(index)}>
            <Text
              style={[
                ...
                categoryIndex === index && styles.categoryTextSelected2,
              ]}
            >
              {item}
            </Text>
    ...
    return (
      <View>
        <View style={{alignItems: "center"}}>
          <Image
            source={welcome}
            style={{marginTop: 40, marginLeft: 18, height: 30}}
          />
          {user.email ? (
            <Text
              style={{
        ...
        <View style={styles.textInfo}>
          ...
          {user.profile?.name ? (
            <Text style={styles.textCat}>{user.profile.name}</Text>
          ) : (
            <Text style={styles.textCat}>Full Name</Text>
          )}
        </View>
        ...
        <View style={styles.textInfo}>
          ...
          {user.profile?.passport ? (
            <Text style={styles.textCat}>{user.profile.passport}</Text>
          ) : (
            <Text style={styles.textCat}>Passport</Text>
          )}
        </View>
        ...
        <View style={styles.textInfo}>
          ...
          {user.profile?.nationality ? (
            <Text style={styles.textCat}>{user.profile.nationality}</Text>
          ) : (
            <Text style={styles.textCat}>Nationality</Text>
          )}
        </View>
        ...
        <View style={styles.textInfo}>
          ...
          />
          {user.profile?.phone ? (
            <Text style={styles.textCat}>{user.profile.phone}</Text>
          ) : (
            <Text style={styles.textCat}>Phone number</Text>
          )}
        </View>
  ...
  return (
    <ScrollView>
      <LinearGradient colors={["#06C5C5", "#14366F"]} style={{height: "100%"}}>
        <Pressable style={styles.containerProfile} onPress={() => pickImage()}>
          {image ? (
            ...
            <View style={styles.containerImage}>
              <Image
                source={avatar}
                style={[styles.avatarImg, {width: 110, height: 110}]}
              />
            </View>
          )}
          <MaterialIcons
